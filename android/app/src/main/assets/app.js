...
document.getElementById('quick-form').onsubmit=event=>{event.preventDefault();openConnection(document.getElementById('address').value.trim(),'shell');};
 ...
 async function selectSiteTab(tab){siteTab=tab;siteTabs.forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));document.getElementById('site-manual').hidden=tab!=='manual';document.getElementById('site-qrcode').hidden=tab!=='qrcode';document.getElementById('site-name').required=tab==='manual';document.getElementById('site-room').required=tab==='manual';document.getElementById('site-secret').required=tab==='manual';if(tab==='qrcode'){window.YourDesk?.requestCameraPermission?.();try{cameraStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}});const video=document.getElementById('site-camera');video.srcObject=cameraStream;await video.play();if('BarcodeDetector' in window){scanner=new BarcodeDetector({formats:['qr_code']});scanTimer=setInterval(async()=>{if(video.readyState<2)return;try{const codes=await scanner.detect(video);if(codes.length&&codes[0].rawValue){document.getElementById('site-qr').value=codes[0].rawValue;document.getElementById('site-status').textContent='已讀取 QR Code，請按儲存站台';clearInterval(scanTimer);scanTimer=null;}}catch{}},250);}else document.getElementById('site-status').textContent='此裝置 WebView 不支援 QR 自動辨識，請貼上掃描結果';}catch{document.getElementById('site-status').textContent='無法取得相機權限';}}else{if(scanTimer){clearInterval(scanTimer);scanTimer=null;}if(cameraStream){cameraStream.getTracks().forEach(t=>t.stop());cameraStream=null;document.getElementById('site-camera').srcObject=null;}}}
