...
document.getElementById('quick-form').onsubmit=event=>{event.preventDefault();openConnection(document.getElementById('address').value.trim(),'shell');};
 ...
 document.getElementById('site-cancel').onclick=()=>{siteDialog.hidden=true;if(cameraStream)cameraStream.getTracks().forEach(t=>t.stop());window.YourDesk?.setSiteDialogVisible?.(false);};
