...
**1.26.0918 build 0754：修正 Windows 畫面正常但鍵鼠無法操作。**補齊 Windows Host 輸入桌面權限，使用者已實測有效；請更新被控 Windows。移除本次排查專用輸入 LOG，並納入 Mac 游標取樣、串流停滯偵測與預設關閉的閒置自動關閉／實驗性自動重連設定。詳見 [本版說明](docs/RELEASE-1.26.0918-build-0754.md)。
