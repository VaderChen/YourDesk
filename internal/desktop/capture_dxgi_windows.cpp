...
struct yd_dxgi_capture {
 ...
 bool captured=false,pending=false;
 ~yd_dxgi_capture(){release();}
 void release(){
  if(pending){context->Unmap(staging.Get(),0);pending=false;mapped={};}
 }
 ...
 HRESULT take(int w,int h){
  if(w!=width || h!=height)return E_INVALIDARG;
  if(pending)return E_UNEXPECTED;
  ...
  pending=true;
  if(!mapped.pData || mapped.RowPitch<(UINT)w*4){this->release();return E_FAIL;}
  // Map 已等待 GPU 複製完成。離開前 ReleaseFrame；只保留我們自己的 staging mapping。
  return S_OK;
 }
 HRESULT copy(unsigned char *rgba,int w,int h){
...
extern "C" int yd_dxgi_take(yd_dxgi_capture *value,int w,int h)try {
 if(!value)return E_INVALIDARG;return value->take(w,h);
}catch(const std::bad_alloc&){return E_OUTOFMEMORY;}catch(...){return E_FAIL;}
extern "C" int yd_dxgi_copy(yd_dxgi_capture *value,unsigned char *rgba,int w,int h)try {
 if(!value)return E_INVALIDARG;return value->copy(rgba,w,h);
}catch(const std::bad_alloc&){return E_OUTOFMEMORY;}catch(...){return E_FAIL;}
extern "C" void yd_dxgi_release(yd_dxgi_capture *value){if(value)value->release();}
