...
Tray 選單會出現「開啟 MCP 遠端畫面」，開啟後切換為「隱藏 MCP 遠端畫面」，可反覆切換，不必中斷連線。若有多個 MCP 連線，選單一次切換這些畫面。首次接管使用者手動開啟的連線時保留可見視窗，不因 MCP 背景偏好自動隱藏；此偏好僅決定 MCP 新建連線的初始顯示方式。Tray 狀態由 顯示區域 接受 show／hide 後的回覆同步（不代表原生視窗動畫已完成），MCP 直接送出的 show／hide 也會同步。

## 遠端操作順序

`remote_action` 的 `show`／`hide` 是本機 顯示區域 視窗操作，不等待首張影格或下一次影像更新；其餘截圖、輸入與資料操作仍須符合連線／畫面就緒檢查。背景 顯示區域 收到 show 後先建立視窗；show／hide 不會暫停或恢復串流。暫停連線的 connected 表示控制通道及模式確認完成，不代表已有畫面；請先呼叫 snapshot。
