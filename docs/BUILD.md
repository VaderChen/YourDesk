...
TurboJPEG 的 x64 NASM 組語建置使用 `--reproducible`，避免 COFF 來源檔名欄位留下本機路徑；請使用 NASM 2.16 或更新版本。此旗標納入快取識別，舊快取不會覆寫或略過路徑驗證。
...
若出現 `No Keychain password item found for profile`，表示目前使用者的 Keychain 找不到指定名稱的公證設定；Developer ID 簽章憑證與公證登入設定是兩件事。建置會在編譯及清理 `dist` 前停止。

在本機終端機建立設定（將 `PROFILE_NAME` 換成錯誤訊息中的名稱）：

```sh
xcrun notarytool store-credentials 'PROFILE_NAME'
```

依互動提示輸入 Apple ID、Team ID 與 app 專用密碼；app 專用密碼可在 [Apple 帳號管理](https://account.apple.com/) 建立。認證資料只存入本機 Keychain，不要寫入專案或聊天內容。若已有其他名稱的設定，可直接使用該名稱：

```sh
YOURDESK_NOTARY_PROFILE='PROFILE_NAME' ./buildMac.command
```

公證憑證不存在、Apple 驗證失敗或網路異常時，均維持停止建置，不跳過公證。
...
每次 Release 前須在本機準備專案根目錄的 `release_zh-Hant.note`、`release_en.note`、`release_ja.note`、`release_ko.note`。這些檔案不提交到 GitHub 原始碼；`pack` 會驗證每檔 1 至 3 行面向使用者的簡短更新重點，並將它們作為 Release 附加資產。新版首次啟動時顯示對應語言的重點，確認後清除使用者更新狀態中的暫存內容；不刪除 App／安裝目錄內的更新重點，以維持 macOS 資源簽章有效。macOS 封裝加入更新重點後，須重新簽署外層 App 並驗證、公證。
...
開啟「強制更新」後，「檢查更新」左側會顯示「安裝測試版本」。按下後查詢 GitHub 已公開的 Prerelease，依發布時間選取最新測試版，顯示版本與更新重點供確認；允許安裝相同或較舊版本。確認後沿用目前平台的套件選擇、下載驗證及安裝流程。沒有測試版或對應平台套件時不會改裝正式版。背景檢查不會自動選取測試版，手動「檢查更新」可切回正式版。
