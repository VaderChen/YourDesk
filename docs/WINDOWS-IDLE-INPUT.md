...
更新日期：2026-09-18。狀態：已於 Windows 10 實機確認輸入桌面存取權限缺漏；使用者已確認更新後 APP 的鍵鼠操作恢復，長時間閒置與鎖定情境仍須分別驗證。
...
修正僅補上 `DESKTOP_JOURNALPLAYBACK`（`0x20`），不要求完整桌面權限、不提升帳號權限。實機 API 比較驗證了此權限缺漏；2026-09-18 使用者亦確認更新後 APP 測試有效。這項確認不代表先前所有閒置問題都由此造成。
