...
- After updating, show only OK instead of download controls or stale messages; confirmation only marks highlights as read.
...
- 更新完了後はダウンロードボタンや古い案内を表示せず、OK のみ表示します。確認は既読として記録するだけで、再ダウンロードしません。
...
- 업데이트 완료 후 다운로드 버튼과 이전 안내 대신 확인만 표시합니다. 확인은 읽음으로만 표시하며 다시 다운로드하지 않습니다.
