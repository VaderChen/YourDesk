...
- **전용 데스크톱 파일 전송**: 별도 창에서 원격 파일 탐색, 폴더 생성, 영구 삭제, 전송 일시 정지·재개 및 진행 상황 확인을 지원합니다. macOS와 Windows용 기능이며 Android와 iOS는 포함하지 않습니다. 화면 캡처나 셸을 시작하지 않습니다.
...
2. 원격 사용자의 홈 폴더를 탐색합니다. Finder / 파일 탐색기에서 로컬 파일이나 폴더를 업로드 영역으로 끌어 놓거나 로컬 파일을 선택하세요. 기존 파일을 덮어쓰거나 기존 폴더에 병합하지 않습니다.
3. 폴더를 만들거나 항목을 선택하여 삭제할 수 있습니다. 삭제는 휴지통으로 이동하지 않는 영구 삭제이며, 폴더는 이름 입력과 내용 삭제 확인도 필요합니다. 다운로드할 원격 파일 하나를 선택하고 완료될 때까지 기다린 뒤 창 맨 아래의 네이티브 영역에서 Finder / 파일 탐색기로 끌어 놓으세요. 완료된 다운로드는 시스템 다운로드 폴더의 `YourDesk` 아래에 유지됩니다.
