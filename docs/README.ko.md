...
- **원격 데스크톱과 터미널**: 작업에 따라 원격 데스크톱이나 대화형 터미널을 선택하세요. 터미널은 독립 창으로 열리며 Linux CLI Host에도 연결할 수 있습니다.
- **전용 데스크톱 파일 전송**: 원격 디스크 탐색, 여러 파일 업로드/다운로드, 폴더 생성 및 확인 후 영구 삭제를 지원합니다. 폴더 우선 정렬과 숨김 항목 제외, 직접 저장 폴더 선택, 일시 정지·수동 재개 및 진행률을 제공합니다. macOS와 Windows용이며 화면 캡처나 셸을 시작하지 않습니다.
- **원격 오디오(실험적)**: 기본으로 꺼져 있고 Opus와 기존 영상 품질 설정을 사용합니다. 자동 선택은 양쪽 지원 여부에 따라 AAC 하드웨어, Opus, AAC 소프트웨어, PCM 순서입니다. [오디오 설정](REMOTE-AUDIO.md).
...
이 README는 프로젝트 소스 코드의 기능을 설명합니다. 배포된 패키지에 포함된 기능은 각 Release 설명을 확인하세요.

## 화면 향상과 보간

전송할 화면을 줄이고 로컬에서 확대해 전송 부담을 낮춥니다. 사용자 경험에 따르면 특히 1440p와 4K처럼 원격 해상도가 높을수록 효과가 커집니다. 장치 지원에 따라 FSR 또는 Core ML을 선택할 수 있습니다.

실험적 **2× 보간**은 Apple Silicon과 macOS 13 이상에서 지원합니다. Apple 저지연 보간 또는 RIFE를 선택할 수 있습니다. 자동 모드는 macOS 27 이상을 실행하는 지원 Mac에서 Apple을 우선 사용합니다. 화면 향상과 보간은 기본적으로 꺼져 있습니다. 효과는 환경에 따라 다르며 빠른 움직임이나 작은 글자에서 왜곡이 나타날 수 있습니다.
...
1. 해당 연결의 CMD 또는 GUI 세션을 닫고, CMD와 GUI 사이의 파일 전송 아이콘을 눌러 연결 암호를 입력하세요. 양쪽 컴퓨터 모두 독립 파일 전송 세션을 지원해야 합니다. 미지원 구형 Host는 연결을 거부하며 데스크톱 세션으로 대신 연결하지 않습니다.
2. 원격 Windows는 `C:\`, Mac은 `~/`에서 시작합니다. `..`로 상위 폴더와 원격 사용자 권한으로 접근 가능한 디스크를 탐색합니다. 로컬 파일/폴더를 끌어 놓거나 네이티브 선택 창에서 여러 파일을 선택하세요. 기존 파일을 덮어쓰거나 폴더에 병합하지 않습니다.
3. 체크박스, Cmd/Ctrl, Shift로 복수 선택할 수 있습니다. 일괄 다운로드는 저장 폴더를 한 번 선택해 직접 저장하며 폴더 다운로드는 지원하지 않습니다. 폴더 생성 및 확인 후 영구 삭제도 가능하며 폴더 삭제는 표시된 이름 입력이 필요합니다. 완료된 파일은 선택한 폴더에 유지됩니다.
4. 업로드와 다운로드를 일시 정지하거나 재개하며 진행률, 전송량 / 전체 크기, 속도와 예상 남은 시간을 확인할 수 있습니다. 연결이 끊기면 전송 창을 유지하고 메인 창에서 같은 대상에 다시 연결한 뒤 수동으로 재개하세요. 양쪽 앱과 전송 창이 계속 열려 있어야 하며 앱 종료나 재부팅 후에는 대기열을 유지하지 않습니다. 창을 닫을 때 취소를 확인하지 못하면 창을 유지하므로 취소를 다시 시도하거나 재연결할 수 있습니다.

이 절차는 macOS / Windows 데스크톱용이며 Android / iOS는 포함하지 않습니다. 크기와 대기열 제한, 취소 동작 및 플랫폼별 검증 상태는 [파일 전송 안내](FILE-TRANSFER.md)를 확인하세요. Windows 네이티브 동작과 실제 Finder / 파일 탐색기 드래그 앤 드롭은 플랫폼별 실기기 검증이 필요하며 브라우저 스모크 테스트로 대체할 수 없습니다.

## 자세한 정보

[파일 전송](FILE-TRANSFER.md) · [스트리밍 설정](STREAMING-CONTROLS.md) · [빌드](BUILD.md) · [구조](ARCHITECTURE.md) · [모델](QUICKSRNET.md) · [보간](FRAME-INTERPOLATION.md)
...
**로컬 Mac에서 원격 파일을 붙여넣지 못하는 문제를 수정했으며 사용자가 실제 기기에서 복구를 확인했습니다.**

## 원격 터미널

GUI(데스크톱) 또는 CMD(터미널)를 선택하거나 연결을 더블 클릭하여 이 두 모드 중 하나를 선택하세요. 전용 파일 전송은 더블 클릭 메뉴가 아니라 CMD와 GUI 사이의 별도 아이콘을 사용합니다. 독립 창으로 여러 연결을 사용할 수 있고 미지원 구버전에는 업데이트 안내가 표시됩니다. Linux는 일반 사용자로 실행하세요. `-secret "암호"`를 지원하며 `LC_ALL → LC_MESSAGES → LANG`에 따라 번체 중국어, 영어, 일본어, 한국어를 선택합니다. 미지원 언어는 영어로 표시합니다. 사용법은 ZIP의 README를 확인하세요.
...
이 프로젝트는 자체 [소스 공개·상업적 판매 금지 라이선스](../LICENSE.ko.md)를 적용합니다. 기업 내부 사용을 포함한 비판매 사용, 수정 및 무료 공유를 허용합니다. 판매, 유료 호스팅/SaaS, 유료 서비스 및 유료 제품 통합은 금지하며 판매 또는 유료 제품·서비스 통합을 위한 별도 라이선스를 논의할 수 있으나 별도 서면 합의 전까지는 이 라이선스가 적용됩니다. 타사 구성 요소에는 각자의 라이선스가 적용됩니다. OSI 정의의 오픈 소스 라이선스가 아니며 번체 중국어 LICENSE 전문이 기준입니다. 통상적인 직원 급여는 판매가 아닙니다. 소프트웨어 제공을 통한 광고 수익, 제휴 수수료 및 데이터 수익화도 제한됩니다. 무료·내부 통합도 라이선스 전문을 준수해야 합니다.
