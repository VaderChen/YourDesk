...
- **Dedicated desktop file transfer:** browse remote disks, select multiple files for upload/download, create folders and confirm permanent deletion. Folders appear first and hidden items are omitted. Choose the local download folder directly, with pause, manual resume and progress controls. Available on macOS and Windows without screen capture or a shell.
- **Remote audio (experimental):** off by default, with Opus and the existing video quality settings. Auto prefers AAC hardware, Opus, AAC software, then PCM according to both ends. See [audio settings](REMOTE-AUDIO.md).
...
2. Start at `C:\` on remote Windows or `~/` on Mac. Use `..` to browse parent folders and disks within the remote user's permissions. Drag local files/folders into the upload area or use the native multi-file picker. Existing files are not overwritten and folders are not merged.
3. Select remote items with checkboxes, Cmd/Ctrl or Shift. Batch downloads ask for a local folder once and save files directly there; folder downloads are not supported. You can create folders or permanently delete selected items after confirmation. Folder deletion also requires typing the listed names. Completed downloads stay in the chosen folder.
