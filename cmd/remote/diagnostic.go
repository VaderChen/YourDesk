...
func runBackgroundDiagnostic(url, room, codec string, mode peertransport.Mode) error {
	...
	defer worker.Close()
	peer, err := p2p.NewViewerWithTransport(ctx, sig, mode, func(f p2p.Frame) { stats.received.Add(1); stats.wire.Store(uint32(f.Codec)); worker.TrySubmit(f) }, func(c p2p.Control) {
