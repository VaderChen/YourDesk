...
func (g *game) Update() (err error) {
	...
	defer func() {
		...
		if err == nil && g.idleClose.Expired(time.Now(), viewerCloseWhenIdle.Load(), connected) {
			slog.Info("遠端桌面連續 15 分鐘未操作，自動關閉")
			go g.peer.Close()
	...
	if g.peer != nil {
		...
		case <-g.peer.Done():
			...
			if viewerAutoReconnect.Load() || (g.reconnect != nil && g.reconnect.running) {
				if action := g.windowShortcut(false); action != 0 {
					return g.executeWindowShortcut(action)
				}
				return g.updateReconnect()
			}
...
func main() {
	...
	defer sig.Close()
	if *interactiveAuth {
		resolve := passwords.resolver()
		sig.ResolveSecret = func(ctx context.Context) ([]byte, error) {
	...
	g.peer = connection.peer
	g.clipboard = connection.clipboard
	g.mu.Unlock()
	...
	g.reconnect = newViewerReconnect(ctx, g, *signalURL, *room, secret, *codec, peertransport.Mode(*transport))
	defer g.reconnect.cancel()
